...
async fn handle_call(
    ...
    match args.action {
        MonitorAction::Start => {
            // Boxed for the same E0275 reason as the inner exec future.
            Box::pin(start(
                &session,
                &turn,
                step_context,
...
async fn start(
    ...
    // Box the exec future: its 0.147 type is deep enough that leaving it
    // inline makes the handler future's auto-Send proof recurse past the
    // compiler's limit (E0275 on tokio's semaphore).
    let initial_output = match Box::pin(manager.exec_command(request, &context)).await {
        Ok(output) => output,
        Err(err) => {
