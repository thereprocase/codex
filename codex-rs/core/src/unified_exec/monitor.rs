...
async fn deliver(session: &Weak<Session>, description: &str, body: String) {
    ...
    if let Err(items) = session.inject_if_running(items).await {
        let turn_inputs = items.into_iter().map(TurnInput::ResponseItem).collect();
        // Both idle-wake paths pull in the whole turn-start machinery. Erase
        // their future types to dyn so the Send proof happens once at the
        // coercion instead of recursing through every future that embeds this
        // delivery loop (the spawn, the tool handler) — E0275 otherwise.
        let wake: std::pin::Pin<Box<dyn std::future::Future<
            Output = Result<(), crate::codex_thread::TryStartTurnIfIdleError>,
        > + Send + '_>> = Box::pin(session.try_start_turn_if_idle(turn_inputs));
        if let Err(err) = wake.await {
            // Recover the ResponseItems from the rejected TurnInputs; this
            // delivery path only ever wraps ResponseItem, so nothing is lost.
            let items: Vec<ResponseItem> = err
                .into_input()
                .into_iter()
                ...
                .collect();
            let record: std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send + '_>> =
                Box::pin(session.inject_no_new_turn(items, None));
            record.await;
        }
    }
}
